...
ball3_img = pg.image.load("source/BNK48/Jennis_cc.png").convert_alpha()
ball3_img = pg.transform.scale(ball3_img, (120, 120))
ball3_rect = ball3_img.get_rect(center=(800, 400))

def collide(ball1 , ball2):
    return (ball1.centerx-ball2.centerx)**2 + (ball1.centery-ball2.centery)**2 <= (ball1.width/2+ball2.width/2)**2
...
while running:
    ...
    if running:
        ...
        text_surface = font.render("6831309021, 6831303221, 6831316421", True, white)
        text_rect = text_surface.get_rect()
        ...
        # TODO 15 : ทำให้ลูกบอลชนกันแล้วเด้งในทิศตรงกันข้าม โดยเช็คการชนที่ขอบวงกลม
        if collide(ball1_rect, ball2_rect):
            ball1_speed, ball2_speed = ball2_speed, ball1_speed
            soundeffect.play()
            # theta = math.atan((ball2_rect.centery-ball1_rect.centery)/(ball2_rect.centerx-ball1_rect.centerx))
            # print(theta)

        if collide(ball2_rect, ball3_rect):
            ball2_speed, ball3_speed = ball3_speed, ball2_speed
            soundeffect.play()

        if collide(ball1_rect, ball3_rect):
            ball1_speed, ball3_speed = ball3_speed, ball1_speed
            soundeffect.play()
